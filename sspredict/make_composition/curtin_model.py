...
#
    def __init__(self,V,E,S,c,ep,T,e_order,b,structure,inputfile):
    ...
    def columnheader(self):
        ...
        for e in self.eorder:
            writeheader.append(e)
        for e in ['V_ave','b_ave','E_ave','mu_ave','nu_ave','sum_cnVn^2_b6','tau_y0','Delta_Eb','Delta_sigma_ss']:
            writeheader.append(e)
        return writeheader
    def str_calc(self):
        # average values
        ...
        writedata = []
#writeheader = append(e for e in ['V_ave','b_ave','E_ave','mu_ave','nu_ave','sum_cnVn^2_b6','Ty0','Delta_Eb','Delta_sigma_ss'])
        for e in self.psc:
            writedata.append(e)
        for e in self.c:
            writedata.append(round(float(e),3))
        for e in [aver_V*10**30,aver_b*10**10,aver_E/10**9,aver_S/10**9,aver_Nu,sum_cndVn_b6,Ty0/10**6,dEb*J2eV,delta_ss/10**6]:
            writedata.append(e)
        return writedata
def main():
    # debug
    ss = ss_model(V,E,S,c,ep,T,e_order,structure,inputfile)
